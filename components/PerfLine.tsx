import type { Perf } from '@/lib/content';
...
/** 那次提及之後的漲跌一行；數字是 build 時用日線快取算好的，前端不打 API。 */
export default function PerfLine({ perf }: { perf?: Perf | null }) {
  if (!perf) return <span className="text-[#6b6b6b]">報價暫時抓不到</span>;
  return (
    <>
      <span>
